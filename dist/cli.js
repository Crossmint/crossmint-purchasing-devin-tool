...
program
    ...
    try {
        ...
        const productSource = index_1.sourceRegistry.getSource(source);
        if (!productSource) {
        ...
        if (hasCompleteAddress) {
            ...
            const addressPrompt = await inquirer_1.default.prompt([
                {
                    type: 'confirm',
                    name: 'provideAddress',
            ...
            if (addressPrompt.provideAddress) {
                const addressDetails = await inquirer_1.default.prompt([
                    {
                        type: 'input',
                        name: 'name',
                        message: 'Full name:',
        ...
        const result = await (0, index_1.buyProductWithCrypto)({
            source,
            productIdentifier,
            isUrl,
            apiKey,
        ...
        if (result.status === 'requires-physical-address') {
            ...
            if (privateKey) {
                try {
                    ...
                    const paymentStatusResponse = await (0, index_1.waitForPaymentPreparation)(result.orderId, apiKey);
                    const orderWithPayment = paymentStatusResponse.order || paymentStatusResponse;
                    // Process the payment
                    console.log('Processing payment...');
                    await (0, payment_1.processPayment)(orderWithPayment, privateKey);
        ...
        if (result.status === 'valid') {
            ...
            while (attempts < maxAttempts) {
                attempts++;
                try {
                    const statusResponse = await (0, index_1.getOrderStatus)(result.orderId, apiKey);
                    ...
                    if (paymentStatus === 'completed' || phase === 'complete') {
                        ...
                        try {
                            ...
                            const finalOrderDetails = await (0, index_1.getOrderStatus)(result.orderId, apiKey);
...
program
    ...
    try {
        ...
        const statusResponse = await (0, index_1.getOrderStatus)(options.orderId, apiKey);
